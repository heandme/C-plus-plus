...
**作用**：跳过当前循环中 `continue` 之后的所有语句，立即开始下一次循环迭代
...
int main() {
    for (int i = 1; i <= 10; i++) {
        if (i % 2 == 0) {
            continue;   // i 是偶数时，跳过 cout，直接进入下一次循环
        }
        cout << i << " ";
    }
    cout << endl;
...
> **注意**：在 `while` 循环中使用 `continue` 时，要确保循环变量的更新在 `continue` 之前，否则可能导致死循环。

**break 与 continue 对比**

| 特性     | `break`   | `continue`  |
| ------ | --------- | ----------- |
| 作用     | 终止整个循环    | 跳过本次循环剩余部分  |
| 循环是否继续 | 不再继续      | 继续下一次迭代     |
| 适用场景   | 找到目标后提前退出 | 某些情况下跳过本次处理 |

> 注意事项
> 
> **只影响当前层循环**：在嵌套循环中，`continue` 只跳过最内层循环的本次迭代，不影响外层循环
> 
> **while 循环中小心死循环**
> 
> **`continue` 不能用于 `switch` 语句**（`break` 可以），但可以在 `switch` 内部的循环中使用
> 
> **尽量保持可读性**：过多的 `continue` 可能让代码逻辑难以追踪，适当使用 `if` 条件判断替代

---

#### goto 语句

**作用**：无条件跳转到函数内标记的**标签**位置。`goto` 可以向前跳转（跳过代码）或向后跳转（重复执行代码）
...
`break` 只能跳出当前层循环，`goto` 可以一次性跳出多层循环
...
int main() {
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 10; j++) {
            if (i * j == 42) {
                goto found;   // 找到目标，直接跳出所有循环
            }
        }
    }
    found:
    cout << "找到了满足条件的组合" << endl;
    return 0;
}
```
...
> 因为`goto`太过强大所以这种做法**不推荐**，应该用 `for` 或 `while` 代替，保持代码可读性
> 
> `goto` 会打乱程序的结构化流程，使代码难以阅读和维护。著名的计算机科学家 **Edsger Dijkstra** 曾发表文章《Go To Statement Considered Harmful》（《`goto` 语句有害》），指出过度使用 `goto` 会写出“面条式代码”。
> 
> **不能跳过变量的初始化**：
> 
> ```cpp
...
> **不能跳转到函数外部**：`goto` 只能在同一个函数内跳转，不能跨函数
...
上面这个表格模拟了一个整型数组（假设我们使用 `int` ，占 4 字节）。地址是连续的，每个元素相差 4 个字节

#### 6.1.1.1 下标

下表就是索引的身份证号，在特定数组可以用下表查看数据

现在查看地址`0x08`的数据我们只要查看下标`2`就可以了
...
int main() {
    ...
    cout << endl; 
    int arr2[6] = {1,11,111,1111,11111,111111};
    for(int i = 0;i<6;i++){
        cout << arr2[i] << " ";
    }
    cout << endl; 
    int arr3[] = {2,22,222,2222,22222,222222,2222222};
    for(int i = 0;i<7;i++){
        cout << arr3[i] << " ";
    }
    return 0;
...
> 一般来说，定义数组中的值与变量一样，你可以不去定义，但是可能数组中会包含“垃圾值”，直接使用会导致程序行为不可预测
> 
> 数组名不得使用关键字，不得与现有变量重名
...
using namespace std;
int main() {
    int arr[10] = {12,34,123,43,18,76,67,74,91,53}; 
    cout << "数组arr占用内存空间为: " << sizeof(arr); //40
    return 0;
}
```
...
一个`int`占 `4`字节，一共10个数组，所以`10·4=40`

代码示例-查看数组单个元素占用的内存空间大小
...
int main() {
    int arr[10] = {12,34,123,43,18,76,67,74,91,53}; 
    cout << "数组arr占用内存空间为: " << sizeof(arr) << endl; //40
    cout << "每个元素占用内存空间: " << sizeof(arr[0]); //4
    return 0;
}
```
...
int main() {
    int arr[10] = {12,34,123,43,18,76,67,74,91,53}; 
    cout << "数组arr占用内存空间为: " << sizeof(arr) << endl; //40
    cout << "每个元素占用内存空间: " << sizeof(arr[0]) << endl; //4
    cout << "数组arr长度: " << sizeof(arr) / sizeof(arr[0]); //10
...
int main() {
    ...
    cout << "数组arr占用内存空间为: " << sizeof(arr) << endl; //40
    cout << "每个元素占用内存空间: " << sizeof(arr[0]) << endl; //4
    cout << "数组arr长度: " << sizeof(arr) / sizeof(arr[0]) << endl;; //10
    cout << "数组arr首地址: " << arr << endl; 
    return 0;
}
```
...
int main(){
    ...
    cout << "数组arr占用内存就空间为 " << sizeof(arr) << endl; 
    return 0;
}
```
...
int main(){
    ...
    cout << "数组arr占用内存就空间为 " << sizeof(arr) << endl;
    cout << "数组arr第一行占用内存就空间为 " << sizeof(arr[0]) << endl;  
    return 0;
}
```
...
> 一个较大的程序通常会被拆分成若干个功能独立的函数，每个函数负责完成特定的任务。这样，当需要多次执行相同逻辑时，只需调用函数即可，无需重复编写代码
...
| 组成部分      | 说明                                                       |
| --------- | -------------------------------------------------------- |
| 返回值类型     | 函数返回的数据类型，如 `int`、`double`、`void`（无返回值）等                 |
| 函数名       | 标识函数的名称，遵循标识符命名规则（建议使用动词或动宾结构）                           |
| 参数列表      | 调用函数时传入的数据，格式为 `类型1 参数1, 类型2 参数2, ...`，无参数时可写 `void` 或留空 |
| 函数体       | 花括号 `{}` 内的代码，实现函数的具体功能                                  |
| return 语句 | 返回函数的结果，并结束函数的执行；返回值类型为 `void` 时可以省略                     |
...
|          | `void` 函数           | 非 `void` 函数         |
| -------- | ------------------- | ------------------- |
| **主要目的** | 产生**副作用**           | 产生**值**（也可能有副作用）    |
| **语法要求** | 不需要 `return` 语句     | 必须有 `return` 返回值    |
| **调用方式** | 语句形式：`func();`      | 表达式形式：`x = func();` |
| **可组合性** | 低（不能参与表达式）          | 高（可嵌套、链式调用）         |
| **测试难度** | 较难（需测试副作用）          | 较易（直接比较返回值）         |
| **函数签名** | `void func(params)` | `T func(params)`    |
...
**实参**：调用函数时实际传入的值，会赋值给对应的形参

函数调用会跳转到函数体执行，执行完毕后返回调用位置继续执行

过程(假设两个实参是`3`,`4`)

```mermaid
graph TD
    A[main 函数执行] --> B[调用 add 函数]
    B --> C[将实参 3,5 传递给形参 a,b]
    C --> D[执行 add 函数体]
    D --> E[return 返回值 8]
    E --> F[返回 main 函数继续执行]
```
...
> 注意，在此代码中，两个`sum`是局部变量，独立于函数`add()`与函数`main()`中
...
因为**值传递时，如果形参发生，并不会影响实参**所以说，num1与num2在RAM中并没有被交换

### 7.1.4 函数的声明

**作用**：在函数定义之前告诉编译器函数的存在，使函数可以在定义之前被调用

> 函数的声明可以有多次，但是函数定义只能有一次
